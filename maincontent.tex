...
\begin{itemize}
    \item 由于 \(\sqrt{\Delta}\) 是正实数，且 \(\sqrt{\Delta} \neq 0\)
    \item 公式中“\(\pm\)”产生两个不同的实数值：
    \[
    x_1 = \frac{-b + \sqrt{\Delta}}{2a}, \quad x_2 = \frac{-b - \sqrt{\Delta}}{2a}
    \]
    \item 因为 \(\sqrt{\Delta} > 0\)，显然 \(x_1 \neq x_2\)
\end{itemize}

\begin{itemize}
    \item 此时 \(\sqrt{\Delta} = 0\)，公式退化为：
    \[
    x = \frac{-b \pm 0}{2a} = -\frac{b}{2a}
    \]
    \item 两个根完全重合，称为“重根”
    \item 几何意义：抛物线与\(x\)轴相切
\end{itemize}

\begin{itemize}
    \item 在实数范围内 \(\sqrt{\Delta}\) 无根
\end{itemize}
\end{proof}
